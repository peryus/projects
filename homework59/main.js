...
async function compressFile(filePath) {
  try {
    const compressedPath = await getAvailablePath(
        `${filePath}.gz`
    );
    ...
    throw new Error(`Помилка стиснення файлу: ${error.message}`);
  }
}
...
async function decompressFile(compressedFilePath, destinationFilePath) {
  try {
    const decompressedPath = await getAvailablePath(destinationFilePath);
    ...
    throw new Error(`Помилка розпакування файлу: ${error.message}`);
  }
}
