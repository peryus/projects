...
const toNumber = (balance: string): number =>
    Number(balance.replace('$', '').replace(',', ''));




const phones2000: string[] = users
    .filter((user: User): boolean => toNumber(user.balance) < 2000)
    .map((user: User): string => user.phone);

console.log(phones2000);



const totalBalance: number = users
    .reduce((sum: number, user: User): number => sum + toNumber(user.balance), 0);
...
const richest: User = users.reduce((maxUser: User, user: User): User => {
    if (toNumber(user.balance) > toNumber(maxUser.balance)) {
        return user;
    }
