...
const toNumber = (balance) => Number(balance.replace('$', '').replace(',', ''));
const phones2000 = users
    .filter((user) => toNumber(user.balance) < 2000)
    .map((user) => user.phone);
console.log(phones2000);
const totalBalance = users
    .reduce((sum, user) => sum + toNumber(user.balance), 0);
console.log(totalBalance);
const richest = users.reduce((maxUser, user) => {
